...
class densePool(nn.Module):
    ...
    def __init__(self, kernel_size, stride_size=2):
        ...
        self.avg_pool_right_down = nn.AvgPool2d(
...
class BasicBlock(nn.Module):
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                AA_3D(
                    ...
                    out_channel=self.expansion * planes,
                    kernel_size=1,
                ),
    ...
    def forward(self, x):
        ...
        # print(self.shortcut)
        # print("x: ", x.shape, "short cut:", self.shortcut(x).shape)
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        ####
        self.conv2 = AA_3D(
            in_channel=planes, out_channel=planes, kernel_size=3, padding=1
        )
        ####
        self.bn2 = nn.BatchNorm2d(planes)
